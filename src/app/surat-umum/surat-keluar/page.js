    ...
    return (
        <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
            <div className="max-w-2xl w-full bg-white p-8 rounded-xl shadow-md border border-gray-200">
                ...
                <h1 className="text-3xl font-bold text-slate-800 text-center">Input Surat Keluar</h1>
                <p className="text-center text-slate-600 mb-8">Lengkapi formulir di bawah untuk mencatat surat keluar.</p>

                {state.message && !state.success && (
                    <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative mb-4" role="alert">
                        {/* Memisahkan pesan error menjadi beberapa baris jika ada */}
                        {state.message.split('\n').map((line, i) => <span key={i} className="block">{line}</span>)}
                    </div>
                )}

                <form ref={formRef} action={formAction} className="space-y-6">
                    {/* Input tersembunyi untuk jenisSurat */}
                    <input type="hidden" name="jenisSurat" value="Surat Keluar" />
                    ...
                    <div>
                        ...
                        <input type="text" id="tujuan" name="tujuan" className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500" />
                    </div>
                    <div>
