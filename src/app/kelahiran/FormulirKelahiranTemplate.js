...
const FormulirKelahiranTemplate = forwardRef(({ surat }, ref) => {
            ...
            <td>
                ...
                <table style={{width: '100%', borderCollapse:'collapse'}}><tbody>
                    <tr>
                        ...
                        <td style={{width: '160px'}}><div style={boxStyle}>{s(get(`formData.${prefix}_kabupaten`)) || '\u00A0'}</div></td>
                    </tr>
                    <tr>
    ...
    const tempatDilahirkanOptions = [{val: '1', label: '1. RS/RB'}, {val: '2', label: '2. Puskesmas'}, {val: '3', label: '3. Polindes'}, {val: '4', label: '4. Rumah'}, {val: '5', label: '5. Lainnya'}];
    const jenisKelahiranOptions = [{val: '1', label: '1. Tunggal'}, {val: '2', label: '2. Kembar 2'}, {val: '3', label: '3. Kembar 3'}, {val: '4', label: '4. Kembar 4'}, {val: '5', label: '5. Lainnya'}];
    const penolongKelahiranOptions = [{val: '1', label: '1. Dokter'}, {val: '2', label: '2. Bidan/Perawat'}, {val: '3', label: '3. Dukun'}, {val: '4', label: '4. Lainnya'}];
    ...
    return (
        <div ref={ref} style={{ fontFamily: 'Arial, sans-serif', fontSize: '10pt', color: '#000', padding: '20px', backgroundColor:'white', width: '210mm' }}>
            ...
            <h3 style={{ textAlign: 'center', fontWeight: 'bold', textDecoration: 'underline', margin: '15px 0'}}>FORMULIR SURAT KETERANGAN KELAHIRAN</h3>
            ...
            <Section title="BAYI / ANAK">
                <FieldRow no="1." label="Nama Lengkap" value={s(get('formData.anak_nama'))} />
                <CheckboxRow no="2." label="Jenis Kelamin" options={jkOptions} value={s(get('formData.anak_jk'))} />
                <CheckboxRow no="3." label="Tempat Dilahirkan" options={tempatDilahirkanOptions} value={s(get('formData.anak_tempat_dilahirkan'))} />
                <FieldRow no="4." label="Tempat Kelahiran" value={s(get('formData.anak_tempat_kelahiran'))} />
                <DateRow no="5." label="Hari dan Tanggal Lahir" prefix="anak_tgllahir" />
                <FieldRow no="6." label="Pukul" value={s(get('formData.anak_pukul'))} />
                <CheckboxRow no="7." label="Jenis Kelahiran" options={jenisKelahiranOptions} value={s(get('formData.anak_jenis_kelahiran'))} />
                <FieldRow no="8." label="Kelahiran Ke" value={s(get('formData.anak_kelahiran_ke'))} />
                <CheckboxRow no="9." label="Penolong Kelahiran" options={penolongKelahiranOptions} value={s(get('formData.anak_penolong'))} />
                <FieldRow no="10." label="Berat Bayi (Kg)" value={s(get('formData.anak_berat'))} />
                <FieldRow no="11." label="Panjang Bayi (Cm)" value={s(get('formData.anak_panjang'))} />
            </Section>

            <Section title="IBU">
                <FieldRow no="1." label="NIK" value={s(get('formData.ibu_nik'))} />
                <FieldRow no="2." label="Nama Lengkap" value={s(get('formData.ibu_nama'))} />
                <DateRow no="3." label="Tanggal Lahir" prefix="ibu_tgllahir" showUmur={true} umurVal={s(get('formData.ibu_umur'))} />
                <FieldRow no="4." label="Pekerjaan" value={s(get('formData.ibu_pekerjaan'))} />
                <AddressRow no="5." label="Alamat" prefix="ibu_alamat" />
                <CheckboxRow no="6." label="Kewarganegaraan" options={kewarganegaraanOptions} value={s(get('formData.ibu_kewarganegaraan'))} />
                <DateRow no="7." label="Tgl Pencatatan Perkawinan" prefix="tgl_perkawinan" />
            </Section>

             <Section title="AYAH">
                <FieldRow no="1." label="NIK" value={s(get('formData.ayah_nik'))} />
                <FieldRow no="2." label="Nama Lengkap" value={s(get('formData.ayah_nama'))} />
                <DateRow no="3." label="Tanggal Lahir" prefix="ayah_tgllahir" showUmur={true} umurVal={s(get('formData.ayah_umur'))} />
                <FieldRow no="4." label="Pekerjaan" value={s(get('formData.ayah_pekerjaan'))} />
                <AddressRow no="5." label="Alamat" prefix="ayah_alamat" />
                <CheckboxRow no="6." label="Kewarganegaraan" options={kewarganegaraanOptions} value={s(get('formData.ayah_kewarganegaraan'))} />
            </Section>
            
            { get('formData.pelapor_nik') &&
                <Section title="PELAPOR">
                    <FieldRow no="1." label="NIK" value={s(get('formData.pelapor_nik'))} />
                    <FieldRow no="2." label="Nama Lengkap" value={s(get('formData.pelapor_nama'))} />
                </Section>
             }
             { get('formData.saksi1_nik') &&
                <Section title="SAKSI I">
                    <FieldRow no="1." label="NIK" value={s(get('formData.saksi1_nik'))} />
                    <FieldRow no="2." label="Nama Lengkap" value={s(get('formData.saksi1_nama'))} />
                </Section>
             }
              { get('formData.saksi2_nik') &&
                <Section title="SAKSI II">
                    <FieldRow no="1." label="NIK" value={s(get('formData.saksi2_nik'))} />
                    <FieldRow no="2." label="Nama Lengkap" value={s(get('formData.saksi2_nama'))} />
                </Section>
             }
...
FormulirKelahiranTemplate.displayName = 'FormulirKelahiranTemplate';
export default FormulirKelahiranTemplate;
